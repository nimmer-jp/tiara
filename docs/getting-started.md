...
※ `website` と `examples` はパッケージに含まれません。`docs/` の Markdown は同梱され、Crown 連携などをオフラインで参照できます。
...
[Crown](https://github.com/nimmer-jp/crown) で `html"""…"""` を使うプロジェクトでは、Tiara 側の `html` マクロと名前が衝突します。対処例は **[crown-integration.md](./crown-integration.md)** を参照してください（`import tiara/core except html` と **`tiaraHtml`**、または `import tiara/components` のみでマクロを読み込まない方法）。

改善計画の索引は [improvement-plan-index.md](./improvement-plan-index.md) にあります。
